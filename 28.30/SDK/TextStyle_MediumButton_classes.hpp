#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: TextStyle_MediumButton

#include "Basic.hpp"

#include "CommonUI_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass TextStyle-MediumButton.TextStyle-MediumButton_C
// 0x0000 (0x01A0 - 0x01A0)
class UTextStyle_MediumButton_C : public UCommonTextStyle
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"TextStyle-MediumButton_C">();
	}
	static class UTextStyle_MediumButton_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<UTextStyle_MediumButton_C>();
	}
};
static_assert(alignof(UTextStyle_MediumButton_C) == 0x000010, "Wrong alignment on UTextStyle_MediumButton_C");
static_assert(sizeof(UTextStyle_MediumButton_C) == 0x0001A0, "Wrong size on UTextStyle_MediumButton_C");

}

