#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: OptionsMenu

#include "Basic.hpp"

#include "OptionsMenu_classes.hpp"
#include "OptionsMenu_parameters.hpp"


namespace SDK
{

// Function OptionsMenu.OptionsMenu_C.BndEvt__TabList_Categories_K2Node_ComponentBoundEvent_0_OnTabButtonCreation__DelegateSignature
// (BlueprintEvent)
// Parameters:
// class FName                             TabID                                                  (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class UCommonButtonBase*                TabButton                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, NoDestructor, HasGetValueTypeHash)

void UOptionsMenu_C::BndEvt__TabList_Categories_K2Node_ComponentBoundEvent_0_OnTabButtonCreation__DelegateSignature(class FName TabID, class UCommonButtonBase* TabButton)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("BndEvt__TabList_Categories_K2Node_ComponentBoundEvent_0_OnTabButtonCreation__DelegateSignature");

	Params::OptionsMenu_C_BndEvt__TabList_Categories_K2Node_ComponentBoundEvent_0_OnTabButtonCreation__DelegateSignature Parms{};

	Parms.TabID = TabID;
	Parms.TabButton = TabButton;

	UObject::ProcessEvent(Func, &Parms);
}


// Function OptionsMenu.OptionsMenu_C.BP_OnActivated
// (Event, Protected, BlueprintEvent)

void UOptionsMenu_C::BP_OnActivated()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("BP_OnActivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.BP_OnDeactivated
// (Event, Protected, BlueprintEvent)

void UOptionsMenu_C::BP_OnDeactivated()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("BP_OnDeactivated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.DialogResult_826C59EC4C294BC872A644878B35A13B
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EFortDialogResult                       Result                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class FName                             ResultName                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UOptionsMenu_C::DialogResult_826C59EC4C294BC872A644878B35A13B(EFortDialogResult Result, class FName ResultName)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("DialogResult_826C59EC4C294BC872A644878B35A13B");

	Params::OptionsMenu_C_DialogResult_826C59EC4C294BC872A644878B35A13B Parms{};

	Parms.Result = Result;
	Parms.ResultName = ResultName;

	UObject::ProcessEvent(Func, &Parms);
}


// Function OptionsMenu.OptionsMenu_C.DialogResult_B31FF5BD4C0772716F27509EA83B418E
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// EFortDialogResult                       Result                                                 (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
// class FName                             ResultName                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UOptionsMenu_C::DialogResult_B31FF5BD4C0772716F27509EA83B418E(EFortDialogResult Result, class FName ResultName)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("DialogResult_B31FF5BD4C0772716F27509EA83B418E");

	Params::OptionsMenu_C_DialogResult_B31FF5BD4C0772716F27509EA83B418E Parms{};

	Parms.Result = Result;
	Parms.ResultName = ResultName;

	UObject::ProcessEvent(Func, &Parms);
}


// Function OptionsMenu.OptionsMenu_C.ExecuteUbergraph_OptionsMenu
// (Final, UbergraphFunction, HasDefaults)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UOptionsMenu_C::ExecuteUbergraph_OptionsMenu(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("ExecuteUbergraph_OptionsMenu");

	Params::OptionsMenu_C_ExecuteUbergraph_OptionsMenu Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function OptionsMenu.OptionsMenu_C.Handle Tab Gamepad Config Action Handler States_SAVE
// (Public, BlueprintCallable, BlueprintEvent)

void UOptionsMenu_C::Handle_Tab_Gamepad_Config_Action_Handler_States_SAVE()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("Handle Tab Gamepad Config Action Handler States_SAVE");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.HandleTabGamePadActionsNeedBinds
// (HasOutParams, BlueprintCallable, BlueprintEvent)
// Parameters:
// TArray<class FText>                     ScreenLabels                                           (BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)

void UOptionsMenu_C::HandleTabGamePadActionsNeedBinds(TArray<class FText>& ScreenLabels)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("HandleTabGamePadActionsNeedBinds");

	Params::OptionsMenu_C_HandleTabGamePadActionsNeedBinds Parms{};

	Parms.ScreenLabels = std::move(ScreenLabels);

	UObject::ProcessEvent(Func, &Parms);

	ScreenLabels = std::move(Parms.ScreenLabels);
}


// Function OptionsMenu.OptionsMenu_C.HandleTabGamepadConfigActionHandlerStates
// (Event, Protected, BlueprintCallable, BlueprintEvent)

void UOptionsMenu_C::HandleTabGamepadConfigActionHandlerStates()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("HandleTabGamepadConfigActionHandlerStates");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.LoadTabClasses
// (Public, BlueprintCallable, BlueprintEvent)

void UOptionsMenu_C::LoadTabClasses()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("LoadTabClasses");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.On Override Reset Modal
// (BlueprintCallable, BlueprintEvent)
// Parameters:
// bool                                    bOverride                                              (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UOptionsMenu_C::On_Override_Reset_Modal(bool bOverride)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("On Override Reset Modal");

	Params::OptionsMenu_C_On_Override_Reset_Modal Parms{};

	Parms.bOverride = bOverride;

	UObject::ProcessEvent(Func, &Parms);
}


// Function OptionsMenu.OptionsMenu_C.OnSettingsTabListPopulated
// (Event, Protected, BlueprintEvent)

void UOptionsMenu_C::OnSettingsTabListPopulated()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("OnSettingsTabListPopulated");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.OnShowResetToDefaultModal
// (Event, Protected, BlueprintEvent)

void UOptionsMenu_C::OnShowResetToDefaultModal()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("OnShowResetToDefaultModal");

	UObject::ProcessEvent(Func, nullptr);
}


// Function OptionsMenu.OptionsMenu_C.OnSplitscreenUpdateSizing
// (Event, Protected, BlueprintEvent)

void UOptionsMenu_C::OnSplitscreenUpdateSizing()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("OnSplitscreenUpdateSizing");

	UObject::ProcessEvent(Func, nullptr);
}

}

